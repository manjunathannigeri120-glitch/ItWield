...
describe('Mission Command Center API Endpoints', () => {
  beforeEach(() => {
    mockSupabaseResponses = {
        ...
        'mission_events': { data: [{ id: 'e-1', event_type: 'MISSION_CREATED' }] }, 'mission_plans': { data: [{ id: 'p-1', version: 1, status: 'ACTIVE' }] }, 'mission_plan_steps': { data: [{ id: 's-1', status: 'PENDING' }] }, 'company_memory': { data: [] },
        'approvals': { data: [] }
    };
    mockUserId = 'user-A';
    vi.clearAllMocks();
