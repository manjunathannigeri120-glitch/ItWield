...
export async function generateWorkflow(options: GenerateOptions): Promise<GenerateResult> {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    try {
      ...
      if (statusCode === 401) userMessage = 'Authentication failed. Please check if the OPENROUTER_API_KEY is correct.';
      else if (statusCode === 429) userMessage = 'Quota exceeded. The AI provider account is out of credits or rate limited.';
      else if (statusCode === 403) userMessage = 'OpenRouter moderation or configuration error.';
...
// ─── Mock generator (when OPENROUTER_API_KEY is not set) ─────────────────────────
...
function mockGenerate(prompt: string, connections: SafeConnection[], agents: SafeAgent[]): GenerateResult {
  ...
  return {
    ...
    explanation: '[Development mode] This is a mock-generated workflow because OPENROUTER_API_KEY is not configured.',
    warnings: ['This workflow was generated by a mock — configure OPENROUTER_API_KEY for real AI generation.']
  };
}
